(function($angular, $moment, Hammer){
  ...
  .directive('timeDial', ['$timeout', '$interval', 'SharedService', function($timeout, $interval, SharedService) {
    // helpers
    ...
    return {
	  ...
	  SharedService.sharedObject.value = new Date(scope.time);
        };

        var calibrate = function(time, beat){
          time = !time ? scope.time : time;
          var second = time.seconds();
          SharedService.sharedObject.second = second;
          var minute = time.minutes();
          SharedService.sharedObject.minute = minute;
          var hour = time.hours();
          SharedService.sharedObject.hour = hours;
